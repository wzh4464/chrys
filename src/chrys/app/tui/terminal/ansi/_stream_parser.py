...
type PatternValue = bool | TokenMatch
type NamedTokenMatch = tuple[str, PatternValue]

type ParseResult[ParseType] = Generator[StreamRead | ParseType, Token | ParseType]
...
class ReadUntil[ResultType](StreamRead[ResultType]):
    ...
    def __init__(self, *characters: str) -> None:
        ...
        self._regex = re.compile("|".join(re.escape(character) for character in characters))
...
class ReadPatterns[ResultType](StreamRead[ResultType]):
    ...
    def feed(self, text: str) -> tuple[int, NamedTokenMatch | None]:
        consumed = 0
...
class PatternToken(Token):
    ...
    def __init__(self, name: str, value: PatternValue) -> None:
        self.name = name
        self.value = value
        super().__init__("")
...
class StreamParser[ParseType]:
    ...
    def __init__(self):
        self._gen: ParseResult[ParseType] | None = self.parse()
    ...
    def _feed(self, text: str) -> Iterable[Token | ParseType]:
        ...
        def send(token: Token) -> Iterable[Token | ParseType]:
            if self._gen is not generator:
                return
            current: Token | ParseType = token
            try:
                while True:
                    new_token = generator.send(current)
        ...
        while text and self._gen is generator:
            if isinstance(self._reading, (ReadPattern, ReadPatterns)):
                ...
                if (match := re.search(self._reading.regex, match_text, re.VERBOSE)) is not None:
    ...
    def parse(self) -> ParseResult[ParseType]:
        # Subclasses supply the real grammar; an empty generator has send type `None`.
        yield from ()  # ty: ignore[invalid-yield]
