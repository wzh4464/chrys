...
class TurnCoordinator:
        ...
        *,
        ...
        while True:
            ...
            if host._fsm.is_running():
                workflow = host._requirement_enrichment_workflow or host._requirement_clarification_workflow
    ...
    async def on_user_interrupt(self, _event: UserInterrupt) -> None:
        ...
        clarification_workflow = host._requirement_clarification_workflow
        enrichment_workflow = host._requirement_enrichment_workflow
        if clarification_workflow is not None:
            await clarification_workflow.request_stop()
        if enrichment_workflow is not None:
            await enrichment_workflow.request_stop()
