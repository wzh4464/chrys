...
def _skill_script_dir() -> Path:
    """The skill ships inside the package, so one path serves checkout and wheel alike.

    Resolving through the repository layout (or a force-included copy) only
    ever found the scripts from a source checkout; keeping them under the
    package is what makes `chrys locate` work from an installed wheel.
    """
    ...
    try:
        async with managed_subprocess(
            ...
            *arguments,
            ...
            # Both are PIPE above, so neither is None; the locals are for the type checker.
            ...
            async def _collect_output() -> tuple[bytes, bytes]:
                async with asyncio.TaskGroup() as task_group:
                    stdout_task = task_group.create_task(
                        _read_stream_bounded(stdout_stream, _MAX_SUBPROCESS_OUTPUT_BYTES)
                    )
                    stderr_task = task_group.create_task(
                        _read_stream_bounded(stderr_stream, _MAX_SUBPROCESS_OUTPUT_BYTES)
                    )
                    await process.wait()
    ...
    *,
    ...
    return (
        ...
        == repo_fingerprint(
            ...
            max_files=config.max_files,
            max_file_bytes=config.max_file_bytes,
            exclude=artifacts.manifest_json.parent,
        )
