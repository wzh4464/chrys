...
@pytest.mark.asyncio
async def test_service_rejects_fewer_than_two_valid_proposers(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    model = _FakeModel(_decision())
    original_propose = model.propose
    ...
    assert result.empty_reason == "insufficient_valid_proposals"
